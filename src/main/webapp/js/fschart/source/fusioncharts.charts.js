...
FusionCharts.register('module', ['private', 'modules.renderer.js-zoomline', function () {
    ...
    renderer('renderer.zoomline', {
        ...
        drawGraph: function () {
            ...
            if (crosslineOptions && crosslineOptions.enabled !== 0 && optionsChart.useCrossline === 1) {
            ...
            while (i--) {
                ...
                if (anchors.show && deltalen) {
                    ...
                    for (j = 0, jj = len; j < jj; j++) {
                        anchors[j] = anchors[j] && anchors[j].show() || paper.circle(attrAnchor, plot.anchorGroup);
                    }
        ...
        drawPlotZoomlineGraphics: function (info, data, element, anchors, labels) {
            var smartLabel = this.smartLabel,
                ...
                useCrossline = chartOptions.useCrossline,
                path = [],
                ...
                parsedLabel,
                tooltipText,
                tooltipTextSeparator = chartOptions.tooltipSepChar;
            ...
            for (; i <= ii; i += step, p += step) {
                parsedLabel = this.getParsedLabel(i);

                if (!useCrossline) {
                    // Initial tooltip text
                    tooltipText = parsedLabel + tooltipTextSeparator + numberFormatter.yAxis(data[i]);

                    // Add plot name if exists in tooltip text
                    tooltipText = info.plotName && (info.plotName + tooltipTextSeparator + tooltipText) || tooltipText;
                }
                d = c / 3 + nulls;
                px = p * ppp;
                lx = px - offset;
                ...
                // Add tooltip only if enabled.
                if (useCrossline) {
                    anchor && (anchor = anchor.attr((attr.cx = ax, attr.cy = y, attr)).next);
                } else {
                    anchor && (anchor = anchor.attr((attr.cx = ax, attr.cy = y, attr)).tooltip(tooltipText).next);
                }
    ...
    CrossLine.prototype.updateLabels = function () {
        ...
        labels && labels.forEach(function (label, i) {
            ...
            label.attr({
                text: numberFormatter.xAxis(value)
            });

            // Now calculate boundingBox and label positions
            bBox = label.getBBox();
            labelWidth = bBox.width;
            halfLabelWidth = labelWidth * 0.5;
            adjustedHalfLabelWidth = halfLabelWidth + GUTTER;
            labelHeight = bBox.height;
            labelYPos = (value === UNDEF || !plot.visible) ? plotOut : (value - yminValue) * pvr;

            // recalculate labelYPos for edge cases.
            // if label is at top edge of chart
            if (labelYPos < ((crossline.height - labelHeight) * -1)) {
                labelYPos += labelHeight;
            }
            // if label is at bottom edge of chart
            else if (labelYPos > ((yminValue * pvr) - labelHeight)) {
                labelYPos -= labelHeight;
            }

            label.attr({
                x: mathMax(0, mathMin(flooredX, visw)),
                y: labelYPos,
